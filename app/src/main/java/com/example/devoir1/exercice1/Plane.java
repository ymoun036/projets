...
public class Plane extends AirTransport {
    private static final double POIDS_MAX = 70000;
    private static final double VITESSE_MAX = 900;
    private static int count = 0;

    public Plane() {
        super(POIDS_MAX * 0.3, VITESSE_MAX * 0.3);
        count++;
        System.out.printf("Default constructor completed creation instance %d of Plane.%n", count);
    }

    public Plane(double poids_vide, double vitesse_croisiere) {
        ...
        count++;
        System.out.printf("Parametrized constructor of Plane completed creation of instance %d with emptyWeight=%.2f, cruisingSpeed=%.2f%n",
                count, poids_vide, vitesse_croisiere);
    }

    @Override
    public String getTransportType() { return "Plane"; }
